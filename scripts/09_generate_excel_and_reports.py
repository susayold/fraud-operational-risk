...
def build_excel(uat: pd.DataFrame, sit: pd.DataFrame) -> None:
    ...
    readme_rows = [
        ...
        ["Observed layer", "PCA fraud benchmark: predictive performance and realised amount capture"],
    ...
    style_sheet(threshold_ws, "OBSERVED PCA BENCHMARK - REALISED ECONOMICS WITH CENTRAL ASSUMPTIONS")
    ...
    executive_rows = [
        ...
        ["Test fraud amount recall", f"{selected['fraud_amount_recall']:.2%}", "Realised amount captured, not guaranteed prevented loss"],
...
Test realised net benefit is **{selected['realised_net_benefit']:,.2f} cost units** under frozen illustrative assumptions. The separate synthetic controls environment creates **{len(alerts):,} investigation alerts**; capacity and SLA results are workload proxies, not observed operations.
